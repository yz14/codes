...
class Trainer(object):
    ...
    def set_model(self):
        """ 模型和损失函数 """
        self.G = G(self.args.d_latent, self.args.dims_G, self.args.d_last_G)
        self.D = D(self.args.d_last_G, self.args.dims_D, self.args.d_last_D)
        self.D.to(self.args.dev)
        self.G.to(self.args.dev)

    def fit(self, start_epoch=0):
        st, ed = start_epoch+1, start_epoch+self.args.epochs+1
        losses = []
    ...
    def training(self, optimD, optimG, losses, epoch):
        ...
        for i, (x,_) in enumerate(self.train_loader):
            ...
            x = x.to(self.args.dev).reshape(bsz, -1)
            x_fake = self.G(noise.to(self.args.dev).float())
...
def get_config():
    ...
    parser.add_argument('--save_root', type=str, default='D:/codes/self-projects/gan_mlp')
    ...
    parser.add_argument('--dims_G', type=list, default=[256, 512], help='生成器dim')
    parser.add_argument('--d_last_G', type=int, default=1024, help='dim')
    parser.add_argument('--dims_D', type=list, default=[512, 256], help='判别器dim')
    parser.add_argument('--d_last_D', type=int, default=1, help='dim')
